...
class YiDongLoader(Dataset):
    ...
    def __getitem__(self, index):
        index = index * self.step
        # if self.flag == "train":
        #     outter_index = index // ((self.train.shape[1] - self.win_size) // self.step + 1)
        #     inner_index = index % ((self.train.shape[1] - self.win_size) // self.step + 1)
        ...
        # elif (self.flag == 'val'):
        #     outter_index = index // ((self.val.shape[1] - self.win_size) // self.step + 1)
        #     inner_index = index % ((self.val.shape[1] - self.win_size) // self.step + 1)
        ...
        # elif (self.flag == 'test'):
        #     outter_index = index // ((self.test.shape[1] - self.win_size) // self.step + 1)
        #     inner_index = index % ((self.test.shape[1] - self.win_size) // self.step + 1)
...
class IOCaseDataLoader(Dataset):
    def __init__(self, root_path, win_size, step=10, flag="train"):
        self.flag = flag
        self.step = step
        self.win_size = win_size
        
        self.scaler = StandardScaler()
        with open(root_path + "/train.json",'r')as f:
            data = json.load(f)
        # print(data.keys())
        # print(data['metircs'])
        remove_metrics = ['guest_nice_usage:','block:','steal','irq_poll:','hrtimer','tasklet',\
                          'softirq_usage','timer','net_rx','rcu:','sched:','nr_slab_unreclaimable','nr_zone_active_anon']
        detect_metrics = []
        for m in data['metircs']:
            need_rm = 0
            for rm in remove_metrics:
                if rm in m:
                    ...
                    break
            if not need_rm:
                detect_metrics.append(m)
        indices =  np.array([i for i, s in enumerate(data['metircs']) if s in detect_metrics])

        filepath = root_path + '/string_array.json'
        print(filepath)
        if not os.path.isfile(filepath):
            with open(filepath, 'w') as file:
                json.dump(detect_metrics, file)
                print('len metrics',len(detect_metrics))


        train_data = np.array(data['data'][0]).T[:,indices]
        with open(root_path + "/test.json",'r')as f:
            data = json.load(f)
        test_data = np.array(data['data'][0]).T[:,indices]
        ...
        print("test:", self.test.shape, "test_labels: ", np.array(self.test_labels).shape)
        print("train:", self.train.shape)
        mask = np.isnan(self.train)
        filled_arr = np.where(mask, np.roll(self.train, 1), self.train)
        self.train = filled_arr
        print('train NA:',np.sum(np.isnan(np.float32(self.train))))
        print('test NA:',np.sum(np.isnan(np.float32(self.test))))
    ...
    def __len__(self):
        ...
        #     return ((self.test.shape[1] - self.win_size) // self.step + 1) * self.test.shape[0]
        if self.flag == "train":
            return (self.train.shape[0] - self.win_size + 1 ) // self.step 
        elif (self.flag == 'val'):
            return (self.val.shape[0] - self.win_size + 1 ) // self.step 
        elif (self.flag == 'test'):
            return (self.test.shape[0] - self.win_size + 1 ) // self.step 
        else:
            return (self.test.shape[0] - self.win_size + 1 ) // self.win_size 
    ...
    def __getitem__(self, index):
        index = index * self.step
        if self.flag == "train":
            # print(np.sum(np.isnan(np.float32(self.train[index:index + self.win_size]))))
            return np.float32(self.train[index:index + self.win_size]),np.float32(self.test_labels[0:self.win_size]), np.float32(self.test_labels[0:self.win_size])
        elif (self.flag == 'val'):
            return np.float32(self.val[index:index + self.win_size]),np.float32(self.test_labels[0:self.win_size]), np.float32(self.test_labels[0:self.win_size])
        elif (self.flag == 'test'):
            return np.float32(self.test[index:index + self.win_size]), np.float32(
                self.test_labels[index:index + self.win_size]),np.float32(
                self.test_labels[index:index + self.win_size])
        else:
            return np.float32(self.test[
                              index // self.step * self.win_size:index // self.step * self.win_size + self.win_size]), np.float32(
...
class XStoreLoader(Dataset):
    def __init__(self, root_path, win_size, step=1, flag="train"):
        ...
        with open(root_path + "/train.json",'r')as f:
            data = json.load(f)
        train_data = np.array(data['data'][0]).T
        with open(root_path + "/test.json",'r')as f:
            data = json.load(f)
        test_data = np.array(data['data'][0]).T
        label = np.array(data['label'][0])
...
#         test_data = np.load(os.path.join(root_path, "test_data.npy"))
#         self.test = self.scaler.transform(test_data.reshape(-1, data.shape[-1])).reshape(self.entity_num, -1, data.shape[-1])
#         self.train = data.reshape(self.entity_num, -1, data.shape[-1])
#         self.val = self.test
#         self.test_labels = np.load(os.path.join(root_path, "label.npy"))
#         print("test:", self.test.shape, "test_labels: ", self.test_labels.shape)
#         print("train:", self.train.shape)
...
#             return ((self.test.shape[1] - self.win_size) // self.step + 1) * self.test.shape[0]
#         # else:
#         #     return (self.test.shape[1] - self.win_size) // self.win_size + 1
...
#             outter_index = index // ((self.train.shape[1] - self.win_size) // self.step + 1)
#             inner_index = index % ((self.train.shape[1] - self.win_size) // self.step + 1)
#             return np.float32(self.train[outter_index, inner_index:inner_index + self.win_size, :-1]), np.float32(
#                 self.test_labels[outter_index, 0:0 + self.win_size]), np.float32(self.train[outter_index, inner_index + self.win_size -1 , -1])
#         elif (self.flag == 'val'):
#             outter_index = index // ((self.val.shape[1] - self.win_size) // self.step + 1)
#             inner_index = index % ((self.val.shape[1] - self.win_size) // self.step + 1)
#             return np.float32(self.val[outter_index, inner_index:inner_index + self.win_size, :-1]), np.float32(
#                 self.test_labels[outter_index, 0:0 + self.win_size]), np.float32(self.val[outter_index, inner_index + self.win_size -1 , -1])
#         elif (self.flag == 'test'):
#             outter_index = index // ((self.test.shape[1] - self.win_size) // self.step + 1)
#             inner_index = index % ((self.test.shape[1] - self.win_size) // self.step + 1)
#             return np.float32(self.test[outter_index, inner_index:inner_index + self.win_size, :-1]), np.float32(
#                 self.test_labels[outter_index, inner_index:inner_index + self.win_size]), np.float32(self.test[outter_index, inner_index + self.win_size -1 , -1])
