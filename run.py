...
parser.add_argument('--c_out_im', type=int, default=7, help='output size')
parser.add_argument('--c_out_time', type=int, default=7, help='output size')
...
if args.is_training:
    for ii in range(args.itr):
        ...
        setting = '{}_{}_{}_{}_epoch{}_layer{}ft{}_sl{}_ll{}_pl{}_dm{}_nh{}_el{}_dl{}_df{}_fc{}_eb{}_dt{}_batch{}_LR{}_{}_{}'.format(
        args.task_name,
        args.model_id,
        args.model,
        args.data,
        ...
        exp.model.module.freeze_step1()
        print('>>>>>>>start training1 : {}>>>>>>>>>>>>>>>>>>>>>>>>>>'.format(setting))
        exp.train(setting, writer, 1)
        torch.cuda.empty_cache()

        exp.model.load_state_dict(torch.load(os.path.join('./checkpoints/' + setting, 'checkpoint.pth')))
        exp.model.module.freeze_step2(args)
        print('>>>>>>>start training2 : {}>>>>>>>>>>>>>>>>>>>>>>>>>>'.format(setting))
        exp.train(setting, writer, 2)
        torch.cuda.empty_cache()
